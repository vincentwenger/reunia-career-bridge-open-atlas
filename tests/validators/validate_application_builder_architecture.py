...
BUILDER_ROUTE_ROOT = ROOT / "products/resume_taylor/application_builder_routes"
BUILDER_ROUTE_MODULES = (
    "lifecycle.py",
    "application_context.py",
    "resume_workflow.py",
    "career_translation.py",
...
def read_builder_source() -> str:
    route_files = sorted(BUILDER_ROUTE_ROOT.rglob("*.py"))
    return "\n\n".join([read(BUILDER)] + [read(path) for path in route_files])
...
def validate_static_routing() -> None:
    ...
    require(
        "application_builder_asset('styles.css')" in base_template,
        "Builder stylesheet does not use the Blueprint static endpoint.",
    )
    require(
        "application_builder_asset('app-shell.js')" in base_template,
        "Builder core JavaScript does not use the production-aware Blueprint asset helper.",
    )
    require(
        "components/common_page_styles.html" in base_template,
        "Builder does not inherit the shared page stylesheet foundation.",
    )
    common_styles = read(ROOT / "products" / "reunia" / "templates" / "components" / "common_page_styles.html")
    require(
        "static_asset('css/base.css')" in common_styles,
