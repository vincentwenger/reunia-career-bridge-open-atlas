...
class ApplicationBuilderIntegrationTests(unittest.TestCase):
    ...
    def test_new_user_career_translation_is_one_time_and_job_independent(self) -> None:
        ...
        self.assertNotIn("Alex Morgan", body)
    ...
    def test_builder_and_shared_static_assets_resolve(self) -> None:
        ...
        scoped_scripts = [
            self.client.get("/applications/static/app-shell.js"),
            self.client.get("/applications/static/app-job-discovery.js"),
            self.client.get("/applications/static/app-resume-workflow.js"),
            self.client.get("/applications/static/interview-preparation.js"),
            self.client.get("/applications/static/resume-async-jobs.js"),
        ]
        obsolete_bundle = self.client.get("/applications/static/app.js")
        shared_css = self.client.get("/static/css/base.css")

        self.assertEqual(builder_css.status_code, 200)
        self.assertTrue(all(response.status_code == 200 for response in scoped_scripts))
        self.assertEqual(obsolete_bundle.status_code, 404)
        self.assertEqual(shared_css.status_code, 200)
        self.assertIn("text/css", builder_css.content_type)
        self.assertTrue(all("javascript" in response.content_type for response in scoped_scripts))
    ...
    def test_open_confirmation_application_targets_confirmation_needed_section(self) -> None:
        application_store = self.app.extensions[
            "career_bridge_application_store"
        ]
        application = application_store.create(
            "architecture-validation-user",
            company="Confirmation Employer",
            role="Application Support Lead",
            job_description="Investigate incidents and maintain service continuity.",
            status="preparing",
        ...
        self.assertEqual(response.status_code, 302)
        self.assertTrue(
            response.headers["Location"].endswith("#confirmation-needed"),
