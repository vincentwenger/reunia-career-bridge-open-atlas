...
class NoInventedExperienceRegressionTests(unittest.TestCase):
    ...
    def test_interview_preparation_repairs_ungrounded_personal_introduction(self) -> None:
        evidence_by_id = {
            "E1": (
                "Designed and developed regulatory reporting solutions using "
                "AxiomSL ControllerView for banking clients."
            ),
            "E2": (
                "Applied SQL and Python data transformations across Oracle and "
                "PostgreSQL environments."
            ),
        }
        workspace = InterviewPreparationWorkspace(
            role_summary="Prepare for an AML data engineering role.",
            company_summary="Only the supplied posting is summarized.",
            expected_responsibilities=[],
            likely_technical_questions=[],
            likely_behavioral_questions=[],
            ...
            questions_to_ask=[],
            personal_introduction=PersonalIntroductionOutline(
                opening="I am a global AML operations leader.",
                current_value="I lead enterprise-wide AML modernization programs.",
                relevant_background="I have extensive sanctions investigations experience.",
                role_connection="This makes me an ideal Global AML Operations Data Engineer.",
                closing="I am ready to transform the company's AML program.",
                evidence_ids=["E1", "E2"],
            ),
        )
        ...
        self.assertEqual(grounded.personal_introduction.evidence_ids, ["E1", "E2"])
        self.assertEqual(
            grounded.personal_introduction.opening,
            evidence_by_id["E1"],
        )
        self.assertEqual(
            grounded.personal_introduction.current_value,
            evidence_by_id["E2"],
        )
        self.assertIn(
            "without adding unsupported experience",
            grounded.personal_introduction.role_connection,
        )

    def test_interview_preparation_recovers_when_model_omits_introduction_ids(self) -> None:
        evidence_by_id = {
            "summary": "Software engineer specializing in regulatory reporting solutions.",
            "EXP-001-B01": "Developed SQL data transformations for banking reports.",
        }
        workspace = InterviewPreparationWorkspace(
            role_summary="Prepare for the target role.",
            company_summary="Only the supplied posting is summarized.",
            ...
            questions_to_ask=[],
            personal_introduction=PersonalIntroductionOutline(
                opening="Experienced candidate.",
                current_value="Relevant background.",
                relevant_background="Strong experience.",
                role_connection="Connect the background to the role.",
                closing="Express interest.",
                evidence_ids=[],
            ),
        )
        ...
        self.assertEqual(
            grounded.personal_introduction.evidence_ids,
            ["summary", "EXP-001-B01"],
        )
        self.assertEqual(
            grounded.personal_introduction.opening,
            evidence_by_id["summary"],
        )
