...
class TrustActionPaletteContractTests(unittest.TestCase):
    def test_shared_palette_tokens_are_present(self):
        content = TOKENS.read_text(encoding="utf-8").lower()
        expected = {
            "--cb-color-primary: #1d4e78",
            "--cb-color-teal: #0f766e",
            "--cb-color-emerald: #137a58",
            "--cb-color-cta: #c2410c",
            "--cb-color-canvas: #f8fafc",
        }
    ...
    def test_primary_cta_is_distinct_from_navigation_anchor(self):
        content = TOKENS.read_text(encoding="utf-8").lower()
        primary = re.search(r"--cb-color-primary:\s*(#[0-9a-f]{6})", content).group(1)
        cta = re.search(r"--cb-color-cta:\s*(#[0-9a-f]{6})", content).group(1)
