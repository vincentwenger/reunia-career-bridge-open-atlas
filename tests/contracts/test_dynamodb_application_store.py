...
class DynamoDBApplicationStoreTests(unittest.TestCase):
    ...
    def test_application_materials_share_the_canonical_application_id(self) -> None:
        application = self.store.create(
            "owner-1",
            company="Example",
            role="Engineer",
        )
        saved = self.store.save_application_materials(
            "owner-1",
            application.id,
            payload_json=(
                '{"application_id":"%s","library_file_ids":["file-1"],'
                '"recruiter_contacts":["Recruiter"]}' % application.id
            ),
        )
        self.assertEqual(saved.application_id, application.id)
        self.assertEqual(saved.owner_id, "owner-1")
        self.assertEqual(saved.payload()["library_file_ids"], ["file-1"])

        item_key = ("owner-1", f"APPLICATION_MATERIALS#{application.id}")
        stored_item = self.table.items[item_key]
        self.assertEqual(stored_item["application_id"], application.id)
        self.assertNotIn("payload_json", stored_item)
        payload_key = stored_item["payload_json_key"]
        self.assertIn(payload_key, self.documents.items)

        self.assertEqual(
            self.store.set_active_application_id("owner-1", application.id),
            application.id,
        )
        self.assertEqual(
            self.store.get_active_application_id("owner-1"), application.id
        )
        with self.assertRaisesRegex(ValueError, "does not exist"):
            self.store.set_active_application_id("owner-1", "missing")

        self.assertTrue(self.store.delete("owner-1", application.id))
        self.assertNotIn(item_key, self.table.items)
        self.assertIn(payload_key, self.documents.deleted)
        self.assertEqual(self.store.get_active_application_id("owner-1"), "")

    def test_mock_interview_sessions_use_application_store_and_cascade(self) -> None:
        application = self.store.create(
            "owner-1",
            company="Example",
            role="Engineer",
        )
        saved = self.store.save_mock_interview_session(
            "owner-1",
            "mock-session-1",
        ...
        self.assertEqual(application.id, saved["application_id"])
        self.assertEqual("active", saved["payload"]["status"])
        item_key = ("owner-1", "MOCK_INTERVIEW_SESSION#mock-session-1")
        stored_item = self.table.items[item_key]
        payload_key = stored_item["payload_json_key"]
        self.assertIn(payload_key, self.documents.items)
        self.assertIsNone(
            self.store.get_mock_interview_session("owner-2", "mock-session-1")
        )
        ...
        self.assertIn(payload_key, self.documents.deleted)
        self.assertIsNone(
            self.store.get_mock_interview_session("owner-1", "mock-session-1")
        )

    def test_unlinked_mock_interview_session_can_be_deleted_directly(self) -> None:
        self.store.save_mock_interview_session(
            "owner-1",
            "mock-session-2",
            application_id="",
            payload_json=(
        ...
        item = self.table.items[(
            "owner-1", "MOCK_INTERVIEW_SESSION#mock-session-2"
        )]
        payload_key = item["payload_json_key"]
        self.assertTrue(
            self.store.delete_mock_interview_session("owner-1", "mock-session-2")
        )
        self.assertIn(payload_key, self.documents.deleted)
        self.assertFalse(
            self.store.delete_mock_interview_session("owner-1", "mock-session-2")
