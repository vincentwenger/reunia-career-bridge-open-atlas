...
class HomeDashboardContractTests(unittest.TestCase):
    ...
    def test_career_evidence_library_has_real_readiness_status(self) -> None:
        template = TEMPLATE.read_text(encoding="utf-8")
        script = SCRIPT.read_text(encoding="utf-8")
        routes = ROUTES.read_text(encoding="utf-8")

        self.assertIn('id="home-evidence-badge"', template)
        self.assertNotIn('<small>Open</small>', template)
        self.assertIn("evidenceBadge(foundation.evidence_library)", script)
        self.assertIn("Ready · ${itemCount} item", script)
        self.assertIn("def _career_evidence_readiness", routes)
        self.assertIn('"total_count": 3', routes)
        self.assertIn('**evidence_library', routes)
