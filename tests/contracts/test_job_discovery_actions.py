...
class JobDiscoveryActionContractTests(unittest.TestCase):
    ...
    def test_user_ready_source_and_preference_controls_are_present(self) -> None:
        ...
        self.assertIn("normalize_source_configuration", source)
        self.assertIn("normalized_source_identifier", source)
        for route in (
            "/discovery/sources",
    ...
    def test_catalog_hydration_is_deferred_until_after_page_render(self) -> None:
        source = package_source(JOB_DISCOVERY_ROUTES)
        template = (
            TEMPLATE.read_text(encoding="utf-8")
            + TEMPLATE.with_name("_discovery_results_content.html").read_text(encoding="utf-8")
        )
        javascript = (
            ROOT / "products" / "resume_taylor" / "static" / "app-job-discovery.js"
        ).read_text(encoding="utf-8")

        workspace = function_source(WORKSPACE_VIEW, "render_job_discovery_workspace")
        hydration = function_source(
            OPERATION_ROUTES, "hydrate_discovered_jobs_from_shared_catalog"
        )
