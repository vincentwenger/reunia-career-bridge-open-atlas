...
class BaselineRoleProfileUpdateTests(unittest.TestCase):
    def test_reviewed_source_fields_update_baseline_experience_and_preserve_bullet_ids(self) -> None:
        ...
        profile = CandidateProfile(
            name="Alex Morgan",
            contact=ContactInfo(location="Portland", phone="", email="alex.morgan@example.com"),
        ...
        changed = apply_career_role_to_profile(
            profile,
            {
                ...
                "employer": "Northstar Financial Platform",
                "dates": "2011 - Present",
        ...
        self.assertEqual(experience.employer, "Northstar Financial Platform")
...
class StructuredCareerRoleInterfaceContracts(unittest.TestCase):
    ...
    def test_baseline_generation_syncs_roles_and_title_findings_link_to_baseline(self) -> None:
        app_source = application_builder_source()
        template = (
            ROOT
            / "products"
            / "resume_taylor"
        ...
        self.assertIn('/career-translation/roles/<role_id>', app_source)
        ...
        self.assertIn("career_translation_assessment_view(", app_source)
        self.assertIn("proposal", app_source)
        self.assertIn("Review title evidence", template)
        self.assertIn("career_translation_workspace') }}#employment-roles", template)

    def test_baseline_javascript_saves_confirms_removes_and_refreshes_roles(self) -> None:
        javascript = (
            ROOT
            / "products"
            / "resume_taylor"
        ...
        self.assertIn("CareerBridgeBaselineUI.refreshSections", javascript)
        self.assertIn("#baseline-resume-preview-region", javascript)
