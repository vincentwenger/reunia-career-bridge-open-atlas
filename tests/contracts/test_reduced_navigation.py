...
class ReducedNavigationContractTests(unittest.TestCase):
    ...
    def test_live_q_and_a_is_not_in_navigation(self) -> None:
        text = NAVBAR.read_text(encoding="utf-8")
        self.assertNotIn("Live Interview Assistance", text)
        self.assertNotIn("Live Assistance", text)
        self.assertNotIn("/live-qa.html", text)
    ...
    def test_new_group_names_are_available_in_french(self) -> None:
        text = (ROOT / "products" / "reunia" / "static" / "js" / "i18n-fr.js").read_text(encoding="utf-8")
        expected = {
