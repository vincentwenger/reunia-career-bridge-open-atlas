...
Install the development-test dependencies and Playwright Chromium before running browser validation:
...
The required-runtime browser phase includes CSP compatibility coverage for key pages, Job Discovery filter auto-submit, and destructive-action confirmation:

```bash
python -m unittest -v tests.browser.test_csp_compatibility
```

In CI and release validation, require every runtime phase instead of allowing missing dependencies to produce a blocked result:
