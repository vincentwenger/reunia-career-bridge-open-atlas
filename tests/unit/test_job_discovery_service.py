...
class JobDiscoveryServiceTests(unittest.TestCase):
    ...
    def test_evidence_fingerprint_is_stable_when_evidence_order_changes(self) -> None:
        ...
        second_reference = EvidenceReference(
            ...
            field_name="statement",
            label="Evidence two",
            statement="Managed AWS data services",
            verification_status="candidate_confirmed",
        )
        first = CandidateJobProfile(
            verified_skills=("Python", "AWS"),
            evidence_statements=(
                "Built Python data pipelines",
                "Managed AWS data services",
            ),
            evidence_references=(first_reference, second_reference),
        )
        reordered = CandidateJobProfile(
            verified_skills=("AWS", "Python"),
            evidence_statements=(
                "Managed AWS data services",
                "Built Python data pipelines",
            ),
            evidence_references=(second_reference, first_reference),
        )

        self.assertEqual(first.evidence_fingerprint, reordered.evidence_fingerprint)

    def test_cache_only_assessment_rebuilds_fit_for_changed_profile(self) -> None:
        ...
        service = JobDiscoveryService(store=store, job_analyzer=analyzer)
        original = CandidateJobProfile(verified_skills=("Python",))
        updated = CandidateJobProfile(verified_skills=("Python", "AWS"))

        first = service.assess_existing_jobs([stored_job], original)
        rebuilt = service.assess_existing_jobs(
            [stored_job],
            updated,
            analyze_new_jobs=False,
        )

        self.assertEqual(1, len(first.ranked_jobs))
        self.assertEqual(1, len(rebuilt.ranked_jobs))
        self.assertEqual(1, len(analyzer.calls))
        self.assertEqual(
            updated.evidence_fingerprint,
            rebuilt.ranked_jobs[0].fit_snapshot.profile_fingerprint,
        )
        self.assertIsNotNone(
            store.get_fit_snapshot(
                stored_job.owner_id,
                stored_job.id,
                updated.evidence_fingerprint,
                stored_job.description_fingerprint,
            )
        )
