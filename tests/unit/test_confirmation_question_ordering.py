...
def test_template_uses_candidate_facing_display_label() -> None:
    ...
    workflow_source = (
        root / "products/resume_taylor/application_builder_routes/resume_workflow_routes/workspace_view.py"
    ).read_text()

    assert "{{ row.display_id }} — {{ q.question }}" in template
    assert '"display_id": candidate_question_display_label(' in workflow_source
