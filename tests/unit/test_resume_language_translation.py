...
from products.resume_taylor.resume_tailor.resume_language import (
    ResumeTranslationField,
    ResumeTranslationFields,
    build_resume_translation_prompt,
    detect_text_language,
    merge_resume_translation_fields,
    resume_translation_field_payload,
...
class ResumeLanguageTranslationTests(unittest.TestCase):
    ...
    def test_same_language_bypass_rechecks_structured_profile_language(self) -> None:
        app_source = (ROOT / "products/resume_taylor/app.py").read_text(encoding="utf-8")
        helper_start = app_source.index("def _ensure_target_language_profile")
        helper_end = app_source.index("def _backfill_professional_contact_links", helper_start)
        helper_source = app_source[helper_start:helper_end]
        ...
        self.assertIn("state.source_resume_language = profile_language", helper_source)
        self.assertLess(
            helper_source.index("profile_language = detect_text_language"),
            helper_source.index("ai.translate_candidate_profile"),
        )

    def test_field_translation_merge_preserves_verified_structure(self) -> None:
        source = french_profile()
        target = english_translation()
        translated_fields = ResumeTranslationFields(
            fields=[
                ResumeTranslationField(**item)
                for item in reversed(resume_translation_field_payload(target))
            ]
        )

        merged = merge_resume_translation_fields(source, translated_fields)

        self.assertEqual([item.id for item in source.experiences], [item.id for item in merged.experiences])
        self.assertEqual(
            [bullet.id for item in source.experiences for bullet in item.bullets],
            [bullet.id for item in merged.experiences for bullet in item.bullets],
        )
        self.assertEqual(source.experiences[0].employer, merged.experiences[0].employer)
        self.assertEqual(source.education[0].institution, merged.education[0].institution)
        self.assertEqual("Lead Software Engineer", merged.experiences[0].title)
        self.assertIn("Developed 12 regulatory reports", merged.experiences[0].bullets[0].text)
        self.assertEqual([], validate_translated_profile(source, merged, "English"))

    def test_field_translation_merge_rejects_missing_keys(self) -> None:
        source = french_profile()
        fields = resume_translation_field_payload(english_translation())
        incomplete = ResumeTranslationFields(
            fields=[ResumeTranslationField(**item) for item in fields[:-1]]
        )

        with self.assertRaisesRegex(ValueError, "missing keys"):
            merge_resume_translation_fields(source, incomplete)
