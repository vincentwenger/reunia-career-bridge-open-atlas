...
class CandidateJobProfile:
    ...
    def evidence_fingerprint(self) -> str:
        """Return an order-insensitive fingerprint for durable Job Fit reuse.

        Resume and evidence records can be deserialized in a different but
        semantically equivalent order after a container replacement. Job Fit
        must not be invalidated merely because lists were reordered, so every
        evidence collection is canonicalized before hashing.
        """
        ...
        return profile_fingerprint(
            {
                "verified_skills": tuple(
                    sorted(self.verified_skills, key=stable_text_key)
                ),
                "evidence_statements": tuple(
                    sorted(self.evidence_statements, key=stable_text_key)
                ),
                "evidence_references": canonical_references,
                "security_clearances": tuple(
                    sorted(self.security_clearances, key=stable_text_key)
                ),
                "licenses_certifications": tuple(
                    sorted(self.licenses_certifications, key=stable_text_key)
                ),
            }
        )
