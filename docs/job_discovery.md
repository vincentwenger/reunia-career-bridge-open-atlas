...
The table name must use the canonical `careerbridge_` prefix. Do not use the legacy `career-bridge-` prefix; DynamoDB treats it as a different table and provisioning it can duplicate storage.
...
aws dynamodb create-table \
  --table-name careerbridge_job_discovery \
...
Evidence fingerprints are canonical and order-insensitive. Reordering equivalent skills, evidence statements, references, clearances, or certifications during serialization or container replacement does not invalidate Job Fit. When a genuinely changed evidence profile has no current fit snapshot but the posting already has a durable `ANALYSIS` record, Career Bridge automatically rebuilds the deterministic fit snapshot from that cached analysis. The job therefore does not return to **Awaiting assessment**, and no new OpenAI request is made.
...
export CAREER_BRIDGE_JOB_DISCOVERY_TABLE_NAME=careerbridge_job_discovery
