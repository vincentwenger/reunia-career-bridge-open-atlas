...
`reports/validation/final-integration-check-results.json` and the companion text output are generated artifacts. Review the command exit code and the status of every required group; do not rely on an older report bundled with a prior package.
