...
```json
{
  ...
  "legacy_features": {
    "recorder": false,
    "live_qa": false,
    "meeting_sharing": false,
    "meeting_transcripts": false,
    "meeting_knowledge": false,
    "meeting_materials_migration": false
  },
  "async_worker": {
    "status": "healthy",
    ...
    "age_seconds": 12,
    "max_age_seconds": 90,
    "worker_id": "career-bridge-worker-1",
    "state": "idle"
  },
  "application_builder": {
    ...
    "async_job_storage": "dynamodb",
    "async_job_durability": "persistent",
...
Leaving the Lightsail **Command** field empty remains recommended for the public web container so the deployed, reviewed image controls the Gunicorn startup command.

### Retired meeting infrastructure

The long-form recorder worker and queue, public meeting-share service, generic meeting aliases, Live Q&A, and legacy meeting-material migration have been removed from the image. Do not configure `RECORDER_JOBS_BUCKET`, `MEETING_SHARES_TABLE_NAME`, or `CAREER_BRIDGE_ENABLE_LEGACY_*` variables. Redis is needed only when a core rate limiter or analytics cache is configured to use it. Adaptive Mock Interview uses the canonical applications table and the focused short-audio transcription path.

### Background AI worker

Job Discovery assessment, Interview Preparation generation, and long Resume Workflow operations no longer execute inside public HTTP requests. Add a second private Lightsail container using the same image and environment variables, but set its command to:

```text
python -m job_discovery.background_worker --poll --interval 5
```

Do not expose a public endpoint for the worker. The web container writes durable queue records and returns `202 Accepted`; the worker claims them with leases, persists progress, and saves final results. Closing the browser, restarting Gunicorn, or replacing a web node does not stop accepted work. A deployment without this second process can accept jobs, but they remain queued until a worker starts.

The worker publishes a durable heartbeat every 15 seconds by default, including during long AI calls. `/health` reports the latest heartbeat and its age. `validate_lightsail_deployment.py` requires a `healthy` heartbeat no older than `CAREER_BRIDGE_ASYNC_WORKER_MAX_AGE_SECONDS` (90 seconds by default), so a web-only deployment cannot pass release validation.

The async queue can share the Job Discovery DynamoDB table through reserved key prefixes. No new index is required. Large Interview Preparation input snapshots are stored under the private application `async-jobs` S3 prefix rather than inside a DynamoDB item. Add an S3 lifecycle rule that expires those snapshots after the async-job retention window. See [`async-ai-jobs.md`](async-ai-jobs.md) for worker, Lambda, retry, and cancellation details.
...
CAREER_BRIDGE_ASYNC_JOB_STORAGE_BACKEND=memory|dynamodb
...
CAREER_BRIDGE_ASYNC_JOB_STORAGE_BACKEND=dynamodb
CAREER_BRIDGE_DOCUMENT_STORAGE_BACKEND=s3
CAREER_BRIDGE_APPLICATIONS_TABLE_NAME=<careerbridge_... table name>
CAREER_BRIDGE_WORKFLOWS_TABLE_NAME=<careerbridge_... table name>
CAREER_BRIDGE_JOB_DISCOVERY_TABLE_NAME=<careerbridge_... table name>
CAREER_BRIDGE_DOCUMENTS_BUCKET=<explicit bucket name>
```

Production startup fails when application storage is not `dynamodb`, when workflow/document storage is non-durable without the explicit override, when any required table/bucket name is blank, or when a Career Bridge DynamoDB table name does not start with `careerbridge_`. The legacy `career-bridge-` prefix is rejected before provisioning or application startup so it cannot recreate duplicate tables. This check runs inside Réunia's
...
postings, fit snapshots, and assessment progress survive replacement. Core
DynamoDB actions/analytics/support/knowledge storage, document storage, secrets,
and every other enabled Career Bridge safeguard are still required. Redis is required only when a core rate limiter or analytics cache is configured to use it. Startup logs a prominent unsafe
...
Réunia's production gate requires S3 and DynamoDB for its durable core services.
Redis is required only when a Redis-backed core cache or rate limiter is selected. A validated persistent deployment may
therefore use more than one Gunicorn worker or Lightsail node. The current image still defaults to one
worker as a conservative capacity choice, not as a storage requirement.

The canonical Windows deployment entry point is `scripts/deployment/upload_to_lightsail.bat`. The separate numbered variants were removed to avoid drift. Use `scripts/deployment/provision_career_bridge_storage.bat` when only the storage preflight is needed.

The upload script:

1. verifies or provisions the configured Career Bridge storage;
2. builds and pushes the image;
3. checks the current Lightsail deployment and stops if any container command
   override is present;
4. enforces scale `1` only when
   `CAREER_BRIDGE_ALLOW_DEMO_STORAGE_IN_PRODUCTION=true`; and
5. otherwise preserves and reports the configured persistent-service scale.
...
1. `/health` returns HTTP 200 with a recent `async_worker.status=healthy` heartbeat and persistent DynamoDB storage for applications,
