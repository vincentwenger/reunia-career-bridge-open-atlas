...
(function () {
    ...
    const STORAGE_KEY = 'reunia-career-action-plan-v2';
    ...
    async function createAction(payload) {
        ...
        if (state.backendAvailable) {
            created = normalizeAction(await fetchJson(API_ACTIONS, {
                method: 'POST',
                headers: {'Content-Type': 'application/json'},
                body: JSON.stringify(payload)
            }));
        } else {
            created = normalizeAction({
                ...payload,
                action_id: `manual-local-${createId()}`,
                source: 'manual',
                source_label: 'Manual action',
                generated: false,
    ...
    async function deleteAction(action) {
        ...
        try {
            ...
            if (!state.backendAvailable) writeLocalActions();
            render();
    ...
    function migrateLegacyBrowserActions() {
        try {
            const legacy = JSON.parse(window.localStorage.getItem('meeting-assistant-action-center-v1') || 'null');
            if (!legacy || !Array.isArray(legacy.manualActions) || !legacy.manualActions.length) return;
            const knownApplications = new Set(state.applications.map(item => item.id));
            const candidates = legacy.manualActions
                .map(normalizeAction)
                .filter(action => knownApplications.has(action.application_id));
            if (!candidates.length) return;
            Promise.all(candidates.map(action => fetchJson(API_ACTIONS, {
                method: 'POST',
                headers: {'Content-Type': 'application/json'},
                body: JSON.stringify({
                ...
                results.forEach(result => state.actions.push(normalizeAction(result)));
                window.localStorage.removeItem('meeting-assistant-action-center-v1');
                render();
            }).catch(error => console.warn('Legacy action migration failed:', error));
        } catch (error) {
    ...
    function readLocalActions() {
        try {
            ...
            return ensureArray(value).map(normalizeAction);
        } catch (error) {
            return [];
        }
    }

    function writeLocalActions() {
        const manualActions = state.actions.filter(action => !action.generated);
        window.localStorage.setItem(STORAGE_KEY, JSON.stringify(manualActions));
    }
