...
def test_clean_template_contains_styles_without_candidate_content(project_root, profile):
    ...
    assert "linkedin.com/in/career-bridge-demo-candidate" not in relationships
    assert "github.com/career-bridge-demo-candidate" not in relationships
...
def test_export_uses_profile_content_instead_of_template_hardcoding(
    ...
    assert "Alex Morgan" not in body_text
