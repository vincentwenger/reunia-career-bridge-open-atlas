...
_APPLICATION_MATERIALS_PREFIX = "APPLICATION_MATERIALS#"
_RESUME_FINDINGS_PREFIX = "RESUME_FINDINGS#"
_INTERVIEW_PREPARATION_PREFIX = "INTERVIEW_PREPARATION#"
_MOCK_INTERVIEW_SESSION_PREFIX = "MOCK_INTERVIEW_SESSION#"
_IMPACT_PREFIX = "IMPACT#"
_SOURCE_JOB_PREFIX = "SOURCE_JOB#"
_ACTIVE_APPLICATION_KEY = "STATE#ACTIVE_APPLICATION"
...
def _application_materials_key(application_id: str) -> str:
    return f"{_APPLICATION_MATERIALS_PREFIX}{application_id}"
...
def _mock_interview_session_key(session_id: str) -> str:
    return f"{_MOCK_INTERVIEW_SESSION_PREFIX}{session_id}"
...
class DynamoDBApplicationStore:
    ...
    ) -> ApplicationMaterialsRecord | None:
        item = self._get_item(owner_id, _application_materials_key(application_id))
        if item is None:
            return None
        return ApplicationMaterialsRecord(
            application_id=application_id,
            owner_id=owner_id,
            payload_json=self._read_json_document(
                item, "payload_json_key", "payload_json"
            ),
            created_at=str(item.get("created_at") or ""),
            updated_at=str(item.get("updated_at") or ""),
        )
    ...
    def save_application_materials(
        ...
        *,
        ...
        try:
            ...
            raise ValueError("Application materials must be valid JSON.") from exc
        if not isinstance(parsed, dict):
            raise ValueError("Application materials must be a JSON object.")

        now = self._clock()
        storage_key = _application_materials_key(application_id)
        existing_item = self._get_item(owner_id, storage_key)
        existing = self.get_application_materials(owner_id, application_id)
        payload_json_key = self._put_json_document(
            owner_id,
            application_id,
            "application-materials.json",
            json.dumps(parsed, ensure_ascii=False, separators=(",", ":")),
            artifact_type="application-materials",
        )
        self._put_item(
            {
                "owner_id": owner_id,
                "storage_key": storage_key,
                "entity_type": "application_materials",
                "application_id": application_id,
                "payload_json_key": payload_json_key,
                "created_at": existing.created_at if existing else now,
                "updated_at": now,
            }
        )
        if existing_item and existing_item.get("payload_json_key") != payload_json_key:
            self._delete_object_keys(existing_item)
        saved = self.get_application_materials(owner_id, application_id)
        if saved is None:  # pragma: no cover
            raise RuntimeError("The application materials were not saved.")
        return saved
    ...
    def get_mock_interview_session(
        ...
        if not normalized:
            return None
        item = self._get_item(owner_id, _mock_interview_session_key(normalized))
        if item is None:
            return None
        try:
            payload = json.loads(
                self._read_json_document(item, "payload_json_key", "payload_json")
            )
    ...
    def save_mock_interview_session(
        ...
        *,
        ...
        if normalized_application and self.get(
            owner_id, normalized_application, include_resume_bytes=False
        ) is None:
            raise ValueError("The selected application does not exist.")
        try:
            parsed = json.loads(payload_json)
        except (TypeError, json.JSONDecodeError) as exc:
            raise ValueError("Mock interview session must be valid JSON.") from exc
        if not isinstance(parsed, dict):
            raise ValueError("Mock interview session must be a JSON object.")

        storage_key = _mock_interview_session_key(normalized_session)
        existing_item = self._get_item(owner_id, storage_key)
        now = self._clock()
        payload_json_key = self._put_json_document(
            owner_id,
            normalized_application or normalized_session,
            f"mock-interview-{normalized_session}.json",
            json.dumps(parsed, ensure_ascii=False, separators=(",", ":")),
            artifact_type="mock-interview-session",
        )
        self._put_item(
            {
                "owner_id": owner_id,
        ...
        if existing_item and existing_item.get("payload_json_key") != payload_json_key:
            self._delete_object_keys(existing_item)
        saved = self.get_mock_interview_session(owner_id, normalized_session)
        if saved is None:  # pragma: no cover
            raise RuntimeError("The mock interview session was not saved.")
        return saved

    def delete_mock_interview_session(
        self, owner_id: str, session_id: str
    ) -> bool:
        normalized = str(session_id or "").strip()
        if not normalized:
            return False
        storage_key = _mock_interview_session_key(normalized)
        item = self._get_item(owner_id, storage_key)
        if item is None:
            return False
        response = self._table().delete_item(
            Key=self._key(owner_id, storage_key),
        ...
        if not response.get("Attributes"):
            return False
        self._delete_object_keys(item)
        return True

    def get_active_application_id(self, owner_id: str) -> str:
        item = self._get_item(owner_id, _ACTIVE_APPLICATION_KEY) or {}
    ...
    def set_active_application_id(
        self, owner_id: str, application_id: str
    ) -> str:
        normalized = str(application_id or "").strip()
        if normalized and self.get(
            owner_id, normalized, include_resume_bytes=False
        ) is None:
            raise ValueError("The selected application does not exist.")
        self._put_item(
            {
                "owner_id": owner_id,
                "storage_key": _ACTIVE_APPLICATION_KEY,
                "entity_type": "active_application",
    ...
    def delete(self, owner_id: str, application_id: str) -> bool:
        ...
        if application_item is None:
            return False
        active_state = self._get_item(owner_id, _ACTIVE_APPLICATION_KEY) or {}
        active_application_id = str(active_state.get("application_id") or "").strip()
        artifact_items = [
            self._get_item(owner_id, storage_key)
            for storage_key in (
                _application_materials_key(application_id),
                _resume_findings_key(application_id),
                _interview_preparation_key(application_id),
                _impact_key(application_id),
            )
        ]
        mock_interview_items = [
            item
            for item in self._query_prefix(owner_id, _MOCK_INTERVIEW_SESSION_PREFIX)
        ...
        if not response.get("Attributes"):
            return False
        # DynamoDB has no foreign keys, so explicitly cascade linked artifact
        # records. S3 deletions create delete markers when bucket versioning is
        # enabled, allowing operator recovery.
        for storage_key in (
            _application_materials_key(application_id),
            _resume_findings_key(application_id),
            _interview_preparation_key(application_id),
        ...
        if active_application_id == application_id:
            self.set_active_application_id(owner_id, "")
        source_job_id = str(application_item.get("source_job_id") or "")
        if source_job_id:
            self._table().delete_item(
                Key=self._key(owner_id, _source_job_key(source_job_id))
            )
        for item in mock_interview_items:
            self._table().delete_item(
                Key=self._key(owner_id, str(item.get("storage_key") or ""))
            )
        self._delete_object_keys(
            application_item, *artifact_items, *mock_interview_items
        )
        return True
