...
class ResumeTranslationField(StrictModel):
    key: str
    text: str


class ResumeTranslationFields(StrictModel):
    fields: list[ResumeTranslationField]
...
RESUME_TRANSLATION_FIELDS_SYSTEM = """You translate independent resume text fields into one target language under a strict evidence-preservation policy.

Rules:
- Return exactly one translated field for every supplied key, with the key unchanged.
- Translate only the text value. Never add, remove, merge, split, summarize, strengthen, or reinterpret facts.
- Preserve every number, percentage, date fact, technology, product name, acronym, responsibility, and outcome.
- Date display wording may be localized, but months, years, range endpoints, and current-role meaning must remain unchanged.
- Do not translate proper names, official product names, acronyms, certification brands, or technology names embedded in text.
- Do not return duplicate keys or keys that were not supplied.
- Return only the structured list of translated fields."""


def resume_translation_field_payload(profile: CandidateProfile) -> list[dict[str, str]]:
    """Flatten only translatable prose while preserving the verified structure."""
    ...
    add("summary", profile.current_summary)
    for index, value in enumerate(profile.skills.hard_skills):
        add(f"skill:hard:{index}", value)
    for index, value in enumerate(profile.skills.soft_skills):
        add(f"skill:soft:{index}", value)
    for index, value in enumerate(profile.skills.industry_knowledge):
        add(f"skill:industry:{index}", value)

    for experience in profile.experiences:
        add(f"experience:{experience.id}:title", experience.title)
        add(f"experience:{experience.id}:dates", experience.dates)
        for bullet in experience.bullets:
            add(f"bullet:{bullet.id}", bullet.text)

    for index, item in enumerate(profile.education):
        add(f"education:{index}:credential", item.credential)
        add(f"education:{index}:date", item.date)
        add(f"education:{index}:detail", item.detail)

    for item in profile.supplemental_evidence:
        add(f"evidence:{item.id}:statement", item.statement)
        for index, value in enumerate(item.verified_skills):
            add(f"evidence:{item.id}:skill:{index}", value)

    return fields


def build_resume_translation_fields_prompt(
    profile: CandidateProfile,
    *,
    target_language: str,
    target_country: str = "",
) -> str:
    fields = resume_translation_field_payload(profile)
    return f"""Translate every resume field below into {language_name(target_language)} for a resume application in {target_country or 'the target market'}.

Return every key exactly once and unchanged. The application merges the translated text back into the immutable Verified Resume Evidence structure, so do not return a CandidateProfile and do not add structural records.
...
def merge_resume_translation_fields(
    source: CandidateProfile,
    translated: ResumeTranslationFields,
) -> CandidateProfile:
    """Merge translated prose into a source-profile skeleton without structural drift."""

    expected = resume_translation_field_payload(source)
    expected_keys = [item["key"] for item in expected]
    expected_set = set(expected_keys)
    values: dict[str, str] = {}
    duplicates: list[str] = []
    for item in translated.fields:
        key = str(item.key or "").strip()
        if key in values:
            duplicates.append(key)
            continue
        values[key] = str(item.text or "")
    ...
    extra = sorted(set(values) - expected_set)
    if duplicates or missing or extra:
        details: list[str] = []
        if duplicates:
            details.append("duplicate keys: " + ", ".join(sorted(set(duplicates))[:6]))
        if missing:
            details.append("missing keys: " + ", ".join(missing[:6]))
        if extra:
            details.append("unexpected keys: " + ", ".join(extra[:6]))
        raise ValueError(
            "The resume translation field map was incomplete ("
            + "; ".join(details)
            + ")."
        )
    ...
    if "summary" in values:
        repaired.current_summary = values["summary"]

    for index in range(len(repaired.skills.hard_skills)):
        key = f"skill:hard:{index}"
        if key in values:
            repaired.skills.hard_skills[index] = values[key]
    for index in range(len(repaired.skills.soft_skills)):
        key = f"skill:soft:{index}"
        if key in values:
            repaired.skills.soft_skills[index] = values[key]
    for index in range(len(repaired.skills.industry_knowledge)):
        key = f"skill:industry:{index}"
        if key in values:
            repaired.skills.industry_knowledge[index] = values[key]

    for experience in repaired.experiences:
        title_key = f"experience:{experience.id}:title"
        dates_key = f"experience:{experience.id}:dates"
        if title_key in values:
            experience.title = values[title_key]
        if dates_key in values:
            experience.dates = values[dates_key]
        for bullet in experience.bullets:
            key = f"bullet:{bullet.id}"
            if key in values:
                bullet.text = values[key]

    for index, item in enumerate(repaired.education):
        for field_name in ("credential", "date", "detail"):
            key = f"education:{index}:{field_name}"
            if key in values:
                setattr(item, field_name, values[key])

    for item in repaired.supplemental_evidence:
        statement_key = f"evidence:{item.id}:statement"
        if statement_key in values:
            item.statement = values[statement_key]
        for index in range(len(item.verified_skills)):
            key = f"evidence:{item.id}:skill:{index}"
            if key in values:
                item.verified_skills[index] = values[key]
