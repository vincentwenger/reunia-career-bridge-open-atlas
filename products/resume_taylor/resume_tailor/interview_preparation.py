...
- Personal introduction: create a five-part outline and cite all evidence IDs used. Keep the opening, current-value, and relevant-background steps close to the wording of verified evidence rather than introducing broad paraphrases. Phrase the role-connection and closing steps as coaching guidance for connecting that verified evidence to the posting, not as new claims that the candidate already has every experience named in the job description.
"""
...
def _preferred_introduction_evidence_ids(
    *,
    ...
    """Choose substantial evidence for a safe introduction fallback.

    Models occasionally omit evidence IDs or attach valid IDs to wording that is
    too paraphrased for deterministic grounding. Interview Preparation should not
    fail when verified evidence is available; it should fall back to the evidence
    itself.
    """
    ...
    for evidence_id in cited_ids:
        if (
            ...
            and evidence_id in evidence_lookup
    ...
    def rank(item: tuple[str, str]) -> tuple[int, int]:
        evidence_id, text = item
        normalized = " ".join(text.split())
        lower_id = evidence_id.casefold()
        lower_text = normalized.casefold()
        looks_like_contact = (
            "@" in normalized
            or "linkedin" in lower_text
            or "github" in lower_text
            or bool(re.search(r"\b\d{3}[-.() ]+\d{3}[-. ]+\d{4}\b", normalized))
        )
        if looks_like_contact or len(normalized) < 18:
            return (9, -len(normalized))
        if lower_id == "summary":
            return (0, -len(normalized))
        if not lower_id.startswith(
            ("skill-", "experience-", "education-", "submitted-resume-")
        ):
            return (1, -len(normalized))
        if lower_id.startswith("skill-"):
            return (2, -len(normalized))
        if lower_id.startswith("submitted-resume-"):
            return (3, -len(normalized))
        if lower_id.startswith("experience-"):
    ...
    candidates = sorted(
        (
            (evidence_id, text)
            for evidence_id, text in evidence_lookup.items()
            if evidence_id in allowed_ids and evidence_id not in selected
        ),
        key=rank,
    )
    for evidence_id, _ in candidates:
        selected.append(evidence_id)
        if len(selected) >= limit:
            break
    return selected
...
def _repair_personal_introduction(
    introduction: PersonalIntroductionOutline,
    *,
    allowed_ids: frozenset[str],
    evidence_lookup: dict[str, str],
) -> PersonalIntroductionOutline:
    """Keep a useful, evidence-traceable outline instead of failing generation.

    The previous implementation validated all five outline steps as one factual
    candidate claim. Job-specific wording in the role-connection or closing step
    could therefore erase otherwise valid evidence IDs and abort the entire
    workspace. Validate candidate-facing steps independently and deterministically
    repair only the unsafe portions.
    """

    if not evidence_lookup:
        # Preserve the legacy ID-only behavior used by callers that do not provide
        # evidence text. There is nothing available for deterministic repair.
        if not introduction.evidence_ids:
            raise ValueError(
    ...
    valid_cited_ids = [
        evidence_id
        for evidence_id in introduction.evidence_ids
        if evidence_id in allowed_ids and evidence_id in evidence_lookup
    ]
    selected_ids = _preferred_introduction_evidence_ids(
    ...
    selected_texts = [evidence_lookup[evidence_id] for evidence_id in selected_ids]
    ...
    factual_fields = ("opening", "current_value", "relevant_background")
    for index, field_name in enumerate(factual_fields):
        current_value = getattr(introduction, field_name)
        if not valid_cited_ids or not grounded(current_value):
            setattr(
                introduction,
                field_name,
                selected_texts[min(index, len(selected_texts) - 1)],
            )

    if not grounded(introduction.role_connection):
        introduction.role_connection = (
            "Connect the verified experience above to the target role's stated "
            "responsibilities without adding unsupported experience"
        )
    if not grounded(introduction.closing):
        introduction.closing = (
            "Close by expressing interest in discussing how this verified background "
            "could support the role"
        )

    introduction.evidence_ids = selected_ids
    return introduction
...
def restrict_workspace_to_evidence(
    ...
    *,
    ...
    workspace.personal_introduction = _repair_personal_introduction(
        workspace.personal_introduction,
        allowed_ids=allowed,
        evidence_lookup=evidence_lookup,
    )
    return workspace
