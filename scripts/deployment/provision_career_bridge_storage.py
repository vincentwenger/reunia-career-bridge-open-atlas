...
DEFAULT_APPLICATIONS_TABLE = "careerbridge_applications"
DEFAULT_WORKFLOWS_TABLE = "careerbridge_workflows"
DEFAULT_DISCOVERY_TABLE = "careerbridge_job_discovery"
CANONICAL_DYNAMODB_TABLE_PREFIX = "careerbridge_"
LEGACY_DYNAMODB_TABLE_PREFIX = "career-bridge-"
...
def _validate_canonical_table_names(names: StorageNames) -> None:
    """Reject stale table names before an AWS create operation can duplicate data."""

    configured = {
        "CAREER_BRIDGE_APPLICATIONS_TABLE_NAME": names.applications_table,
        "CAREER_BRIDGE_WORKFLOWS_TABLE_NAME": names.workflows_table,
        "CAREER_BRIDGE_JOB_DISCOVERY_TABLE_NAME": names.discovery_table,
    }
    invalid = [
        f"{key}={table_name!r}"
        for key, table_name in configured.items()
        if not table_name.startswith(CANONICAL_DYNAMODB_TABLE_PREFIX)
    ]
    if not invalid:
        return

    legacy_found = any(
        table_name.startswith(LEGACY_DYNAMODB_TABLE_PREFIX)
        for table_name in configured.values()
    )
    explanation = (
        " Legacy 'career-bridge-' names are not valid and can recreate duplicate tables."
        if legacy_found
        else ""
    )
    raise ProvisioningFailure(
        "Career Bridge DynamoDB table names must start with "
        f"{CANONICAL_DYNAMODB_TABLE_PREFIX!r}: "
        + ", ".join(invalid)
        + "."
        + explanation
    )
...
def _resolve_names(args: argparse.Namespace) -> StorageNames:
    ...
    _validate_canonical_table_names(names)
    return names
...
def _enable_table_ttl(
    ...
    *,
    ...
    if result.returncode:
        raise ProvisioningFailure(
            f"Could not inspect TTL on {table_name!r}: "
            f"{result.stderr or result.stdout}"
        )
    ...
    if status in {"ENABLED", "ENABLING"} and attribute == "expires_at":
        print(f"READY {label} TTL: {table_name}.expires_at")
        return
    if status in {"ENABLED", "ENABLING"} and attribute != "expires_at":
        raise ProvisioningFailure(
            f"{label.title()} table TTL is already configured on {attribute!r}; "
            "expected 'expires_at'."
        )
    _aws(
        aws_cli,
    ...
    print(f"ENABLED {label} TTL: {table_name}.expires_at")


def _enable_workflow_ttl(aws_cli: str, names: StorageNames) -> None:
    _enable_table_ttl(
        aws_cli, names, table_name=names.workflows_table, label="workflow"
    )


def _enable_discovery_ttl(aws_cli: str, names: StorageNames) -> None:
    _enable_table_ttl(
        aws_cli, names, table_name=names.discovery_table, label="job discovery"
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if not args.applications_only:
            ...
            _enable_discovery_ttl(args.aws_cli, names)
