...
def _validate_async_worker_health(payload: dict[str, Any]) -> dict[str, Any]:
    worker = payload.get("async_worker")
    if not isinstance(worker, dict):
        raise ValidationFailure(
            "Health endpoint does not expose async_worker heartbeat metadata. "
            "Deploy the worker heartbeat implementation before accepting background jobs."
        )

    status = str(worker.get("status") or "missing").strip().casefold()
    last_heartbeat_at = str(worker.get("last_heartbeat_at") or "").strip()
    age_seconds = worker.get("age_seconds")
    max_age_seconds = worker.get("max_age_seconds")
    if status != "healthy":
        detail = (
            f"status={status!r}, last_heartbeat_at={last_heartbeat_at or '<none>'}, "
            f"age_seconds={age_seconds!r}, max_age_seconds={max_age_seconds!r}"
        )
        raise ValidationFailure(
            "Async worker heartbeat is not healthy; queued AI jobs could remain "
            f"unprocessed ({detail}). Start the private worker with "
            "'python -m job_discovery.background_worker --poll --interval 5' "
            "and rerun deployment validation."
        )

    try:
        age = int(age_seconds)
        maximum = int(max_age_seconds)
    except (TypeError, ValueError) as exc:
        raise ValidationFailure(
            "Async worker heartbeat metadata has invalid age values."
        ) from exc
    if not last_heartbeat_at or age < 0 or maximum < 1 or age > maximum:
        raise ValidationFailure(
            "Async worker heartbeat is stale or incomplete even though it was "
            f"reported healthy (last_heartbeat_at={last_heartbeat_at or '<none>'}, "
...
def _validate_health(
    ...
    *,
    ...
    _validate_async_worker_health(payload)
    ...
    if discovery_table and not discovery_table.startswith("careerbridge_"):
        raise ValidationFailure(
            "Health endpoint reports a non-canonical Job Discovery DynamoDB table "
            f"name ({discovery_table!r}). Table names must start with "
            "'careerbridge_'; legacy 'career-bridge-' names can recreate duplicate tables."
        )
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    try:
        ...
        worker_health = health["async_worker"]
        print(
            "PASS  /health returned 200, status=ok, approved storage "
            f"configuration ({durability}), and a healthy async worker "
            f"heartbeat (age={worker_health['age_seconds']}s)."
        )
